...
/// - Maximum 2 blocks per slot
/// - Maximum 8 blocks in unincluded segment
pub struct CustomFixedVelocityConsensusHook<
	T,
	const RELAY_CHAIN_SLOT_DURATION_MILLIS: u32,
	V: Get<u32>,
	const C: u32,
...
impl<
		...
		V: Get<u32>,
		const C: u32,
	> ConsensusHook for CustomFixedVelocityConsensusHook<T, RELAY_CHAIN_SLOT_DURATION_MILLIS, V, C>
where
	<T as pallet_timestamp::Config>::Moment: Into<u64>,
{
	/// Consensus hook that performs validations on the provided relay chain state
...
impl<
		...
		V: Get<u32>,
		const C: u32,
	> CustomFixedVelocityConsensusHook<T, RELAY_CHAIN_SLOT_DURATION_MILLIS, V, C>
{
	/// Whether it is legal to extend the chain, assuming the given block is the most
	/// recently included one as-of the relay parent that will be built against, and
	/// the given slot.
